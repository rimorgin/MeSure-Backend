...
class HandImageProcessor:
    ...
    def finger_tracking(self, image, hand_label, palm_orientation):
        ...
        if results.multi_hand_landmarks:
            for handType, handLms in zip(results.multi_handedness, results.multi_hand_landmarks):
                ...
                if hand_label == 'Left':
                    if palm_orientation == 'Palm':
                        adjustments = [120, 80, -120, 80]  # Adjustments for left hand palm up
                    elif palm_orientation == 'Back':
                        adjustments = [-100, 80, 100, 80]  # Adjustments for left hand palm down
                elif hand_label == 'Right':
                    if palm_orientation == 'Palm':
                        adjustments = [-100, 80, 100, 80]  # Adjustments for right hand palm up
                    elif palm_orientation == 'Back':
                        adjustments = [120, 80, -120, 80]  # Adjustments for right hand palm down
                ...
                for i, (finger_name, (mcp, pip)) in enumerate(finger_joints.items()):
                    ...
                    midpoint1, _ = self.calculate_midpoint_and_distance(midpoint0[0], midpoint0[1], pip[0], pip[1])
                    ...
                    #print(f'adjustment to use: {adjustments}')
    ...
    def orientation_tracking(self, image):
        # Process the hand image passed as an object.
        ...
        palm_orientation = None  # Initialize palm_orientation
        ...
        if results.multi_hand_landmarks:
            for handType, handLms in zip(results.multi_handedness, results.multi_hand_landmarks):
                ...
                pinky_tip_x = handLms.landmark[20].x

                if hand_label == "Left":
                    if thumb_tip_x < pinky_tip_x:
                        palm_orientation = "Back"
                    else:
        ...
        return hand_label, palm_orientation
...
palm_orientation = processor.detect_palm_orientation(image)
print(f"Palm orientation: {palm_orientation}")
"""
